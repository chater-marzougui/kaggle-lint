...
Current milestone plan: docs/next_plans/milestone-8-user-experience/plan.md
Current branch: main (M7 executed directly on main, no feature branch — user's call; same applies unless stated otherwise for M8)
Current N: 8
Current X–Y: —
...
**M7 status: merged to main (2026-07-10).** Plan: `docs/superpowers/plans/2026-07-10-m7-single-frame-and-navigation.md`. Executed via `superpowers:subagent-driven-development` directly on `main` (no feature branch/worktree — user's explicit call; pending doc changes were committed first at `f941af4` as the branch base). All 4 code tasks (F32 mount gate, uuid plumbing, F33 scroll bridge, F34 store reconciliation) implemented and individually reviewed — zero Critical/Important across all four. Final whole-branch review (Opus): ready to merge, zero Critical/Important, 2 Minor (both transient-state UX nits, not bugs). Commits `94fec6b..b264f99`.

The user's own Task 5 manual gate confirmed the core fixes (single overlay, exact-line scroll, deleted-cell errors clearing) but surfaced three follow-up issues, all fixed same-session and pushed (`6a211d8`, `c40e840`, `f76e01f`, `c10389c`):
- **Rapid engine-switching "crash."** Root-caused (not guessed): F32's fix requires a hard page refresh, not just an extension reload — a stale pre-fix content script left the old dual-frame-mount bug live in the tested tab. Once genuinely single-frame, the remaining flakiness was distinct back-to-back full lint passes (flake8 in particular takes several seconds) stacking up long enough in a row to trip Chrome's own unresponsive-page detector. Fixed by debouncing the settings-changed re-lint (500 ms for an engine switch, 1 s for ignore-codes typing, since typing fires far more rapidly).
- **Click-to-scroll didn't center the line.** First attempt set the CM6 view's own `scrollDOM.scrollTop`, confirmed live to have zero effect — a Jupyter cell's editor auto-grows to fit its content, so there's nothing to scroll *inside* it; the real scroll container is the notebook's own windowed viewport one level up. Fixed by walking up from the target line's real DOM element to whichever ancestor actually has overflow to scroll.
- **Line highlight only appeared when no scroll was needed.** The DOM lookup for the target line ran synchronously in the same tick as the scroll call; under virtualization, a scroll that actually had to move the viewport can defer mounting that line's DOM node by a frame or more. Fixed by polling a bounded number of animation frames for the line element before acting.
- Also unified all four extension log call sites (previously three different hardcoded tag families: `[Kaggle Linter]`, bare `[Linter]`, `[Kaggle Linter DomParser]`/`CodeMirror]`) behind one `createLogger()` in `utils/logger.ts`.

**Important for M8:** Task 1 (the overlay React rewrite) touches `Overlay.tsx`/`Overlay.css`/`packages/ui-components/src/types/index.ts` — all three were already changed by M7 (uuid field, single-scroll-path cleanup that deleted `scrollToError`/`highlightCell`). Read their current state before expanding M8 Task 1, not the M8 plan text's paraphrase of a pre-M7 shape.
...
Current: Read docs/review-findings.md (both addenda), docs/architecture.md, docs/next_plans/README.md, and docs/next_plans/milestone-8-user-experience/plan.md. The milestone plan's decisions are pre-made — do not re-open them, do not brainstorm. Use superpowers:writing-plans to expand it into a full TDD implementation plan at docs/superpowers/plans/〈date〉-m8-user-experience.md, verifying every file path and signature the milestone plan names against real current source — Task 1 in particular touches `Overlay.tsx`/`Overlay.css`/`packages/ui-components/src/types/index.ts`, all of which Milestone 7 already changed (uuid field, deleted `scrollToError`/`highlightCell`), so read their current state, not the milestone plan's paraphrase of a pre-M7 shape. Three things the expansion must respect: (1) Task 3's line→element mapping under virtualization is explicitly an open, live-probe problem, not a solved one — the plan says to probe for a usable isolated-world anchor first and only extend the bridge (additive, following M7's own conventions) if none is reliable; keep the TDD plan probe-first, don't presuppose which mechanism wins; (2) settings storage shape is frozen — Tasks 2 and 4 change values/defaults only, never keys or types; (3) Task 1 (the overlay React rewrite) is the foundation every later task builds on — sequence it first, and treat its resulting interfaces (typed error props, CSS-driven minimize state) as fixed inputs for later tasks' briefs rather than something to re-derive. Keep the plan's Task 7 manual verification gate as the final task, marked USER-GATE. Then stop for my review — do not start implementing.
