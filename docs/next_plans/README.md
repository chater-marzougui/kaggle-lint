...
5. Where a plan and reality disagree (Kaggle DOM changed, Jupyter API shape differs from the plan's expected sketch), the finding's *intent* wins; document the deviation in the milestone folder as `notes.md`. Live-probing `window.jupyterapp` shapes on a real notebook is the established method (M2 precedent).
