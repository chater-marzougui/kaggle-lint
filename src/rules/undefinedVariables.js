...
const UndefinedVariablesRule = (function () {
  ...
  const PYTHON_BUILTINS = new Set([
    ...
    'vars', 'zip', '__import__', '__name__', '__doc__', '__package__',
    '__loader__', '__spec__', '__annotations__', '__builtins__', '__file__',
    '__cached__', 'True', 'False', 'None', 'Ellipsis', 'NotImplemented',
    'Exception', 'BaseException', 'ValueError', 'TypeError', 'KeyError',
    'IndexError', 'AttributeError', 'ImportError', 'RuntimeError',
    'StopIteration', 'GeneratorExit', 'AssertionError', 'ArithmeticError',
    ...
    'RecursionError', 'MemoryError', 'Warning', 'UserWarning',
  ...
  const COMMON_LIBRARIES = new Set([
    ...
    'cv2', 'PIL', 'os', 'sys', 're', 'json', 'csv', 'math', 'random',
    'datetime', 'time', 'collections', 'itertools', 'functools',
    'pathlib', 'glob', 'shutil', 'pickle', 'warnings', 'logging',
    'tqdm', 'requests', 'bs4', 'selenium', 'keras', 'xgboost',
    'lightgbm', 'catboost', 'gc', 'copy', 'io', 'struct', 'typing',
    'subprocess', 'threading', 'multiprocessing', 'queue', 'asyncio'
  ]);
  ...
  function extractDefinedNames(code) {
    ...
    lines.forEach((line, idx) => {
      ...
      match = /^\s*([a-zA-Z_][a-zA-Z0-9_]*(?:\s*,\s*[a-zA-Z_][a-zA-Z0-9_]*)*)\s*=(?!=)/.exec(line);
      ...
      match = /^\s*for\s+([a-zA-Z_][a-zA-Z0-9_]*(?:\s*,\s*[a-zA-Z_][a-zA-Z0-9_]*)*)\s+in\s+/.exec(line);
      if (match) {
        const names = match[1].split(',').map(n => n.trim());
        names.forEach(name => {
          if (/^[a-zA-Z_][a-zA-Z0-9_]*$/.test(name)) {
      ...
      const listCompMatch = /\[.+\s+for\s+([a-zA-Z_][a-zA-Z0-9_]*)\s+in\s+/.exec(line);
      if (listCompMatch) {
